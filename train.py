...
#for e in range(0, 4):
    print('config changed: old values as comments\nBCEwithLogits\nRemovedSigmoid and added before accuracy')
    i = 0
    ...
    for x, y_mask, y_label, y_intensity in trainDataLoader:
        ...
        where = torch.sigmoid(pred[0])
        where = torch.Tensor(torch.where(where> torch.Tensor([config.THRESHOLD]).to(device), 1, 0)).type(torch.uint8)
        metrics["mask_acc"] = np.append(metrics["mask_acc"],float(f.dice(where.to(config.DEVICE), y_mask.type(torch.uint8)).cpu().item()))
    ...
    metrics["mask_train_acc"] = np.append(metrics["mask_train_acc"], metrics["mask_acc"].sum().item() / (config.BATCH_SIZE * len(trainDataLoader)))
    ...
    metrics["mask_train_loss"] = np.append(metrics["mask_train_loss"], metrics["mask_loss"].sum().item() / (config.BATCH_SIZE * len(trainDataLoader)))
    metrics["label_train_loss"] = np.append(metrics["label_train_loss"], metrics["label_loss"].sum().item() /  (config.BATCH_SIZE * len(trainDataLoader)))
    metrics["intensity_train_loss"] = np.append(metrics["intensity_train_loss"],metrics["intensity_loss"].sum().item() / (config.BATCH_SIZE * len(trainDataLoader)))
